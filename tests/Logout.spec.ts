...
myAccountPage.
  ...
  //	"1. User should be taken to the 'Account Logout' page and User should see Login option inplace of Logout under the 'My Account' dropmenu
  //2. User should be taken to the Home page"
});
test("Validate the Application session status, after logging and closing the Browser without logging out	-", async ({}, testInfo) => {
  ...
  //	1. Application should not get logged out, instead the user loggedin session need to be mainitained
});
test("Validate logging out and browsing back	-", async ({}, testInfo) => {
...
test("Validate logging out and loggin in immediately after logout 	-", async ({}, testInfo) => {
  ...
  //	1. Same Account or Differnet Account should get loggedin
});

test("Validate 'Account Logout' page	-", async ({}, testInfo) => {
  ...
  //	1. Proper Page Heading, Page Title, Page URL and Breadcrumb are displayed for 'Account Logout' page
});
...
test("Validate the UI of the Logout option and the 'Account Logout' page	-", async ({}, testInfo) => {
  ...
  //	1. Proper UI adhering to the UI checklist should be displayed for Logout option (My Account DropMenu and Right Column) and 'Account Logout' page
});

test("Validate the Logout functionality in all the supported environments	-", async ({}, testInfo) => {
  setTestCaseId(testInfo, "TC_LG_011");
  expect(await myAccountPage.doesPageExist()).toBeTruthy();
  //	"1. Click on 'My Account' Dropmenu
  // 2. Select 'Logout' option (Verify ER-1)"
  //        ER-1. Logout functionality should work correctly in all the supported environments
  //	1. Logout functionality should work correctly in all the supported environments
});
